...
from aedge.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from aedge.protocols import full_node_protocol
from aedge.protocols.full_node_protocol import RespondBlock
from aedge.rpc.rpc_server import start_rpc_server
from aedge.rpc.wallet_rpc_api import WalletRpcApi
from aedge.rpc.wallet_rpc_client import WalletRpcClient
from aedge.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
from aedge.util.ints import uint16, uint32
from aedge.wallet.derive_keys import master_sk_to_local_sk
...
class TestPoolWalletRpc:
    ...
    async def create_pool_plot(self, p2_singleton_puzzle_hash: bytes32) -> bytes32:
        ...
        try:
            plot_keys = PlotKeys(bt.farmer_pk, None, encode_puzzle_hash(p2_singleton_puzzle_hash, "taec"))
    ...
    async def test_absorb_self(self, one_wallet_node_and_rpc):
        ...
        tr: TransactionRecord = await client.send_transaction(
            1, 100, encode_puzzle_hash(status.p2_singleton_puzzle_hash, "taec")
        )
        await time_out_assert(
            10,
            full_node_api.full_node.mempool_manager.get_spendbundle,
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc):
        ...
        # Balance stars at 6 AEC and 5 more blocks are farmed, total 22 AEC
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            async def have_aedge():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_aedge)
