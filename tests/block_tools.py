...
from aedge.cmds.init_funcs import create_all_ssl, create_default_aedge_config
from aedge.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from aedge.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from aedge.util.errors import Err
from aedge.full_node.generator import setup_generator_args
from aedge.full_node.mempool_check_conditions import GENERATOR_MOD
from aedge.plotting.create_plots import create_plots, PlotKeys
...
from aedge.consensus.block_rewards import calculate_base_farmer_reward, calculate_pool_reward
from aedge.consensus.blockchain_interface import BlockchainInterface
from aedge.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from aedge.consensus.condition_costs import ConditionCost
from aedge.consensus.constants import ConsensusConstants
from aedge.consensus.default_constants import DEFAULT_CONSTANTS
from aedge.consensus.deficit import calculate_deficit
from aedge.consensus.full_block_to_block_record import block_to_block_record
from aedge.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from aedge.consensus.pot_iterations import (
    calculate_ip_iters,
...
from aedge.consensus.vdf_info_computation import get_signage_point_vdf_info
from aedge.full_node.signage_point import SignagePoint
from aedge.plotting.util import PlotInfo, PlotsRefreshParameter, PlotRefreshResult, parse_plot_info
from aedge.plotting.manager import PlotManager
from aedge.server.server import ssl_context_for_server
...
from aedge.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from aedge.types.blockchain_format.pool_target import PoolTarget
from aedge.types.blockchain_format.program import INFINITE_COST
from aedge.types.blockchain_format.proof_of_space import ProofOfSpace
from aedge.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from aedge.types.blockchain_format.sized_bytes import bytes32
from aedge.types.blockchain_format.slots import (
...
from aedge.types.blockchain_format.vdf import VDFInfo, VDFProof
from aedge.types.end_of_slot_bundle import EndOfSubSlotBundle
from aedge.types.full_block import FullBlock
from aedge.types.generator_types import BlockGenerator, CompressorArg
...
from aedge.util.block_cache import BlockCache
from aedge.util.condition_tools import ConditionOpcode
from aedge.util.config import load_config, save_config
from aedge.util.hash import std_hash
from aedge.util.ints import uint8, uint16, uint32, uint64, uint128
from aedge.util.keychain import Keychain, bytes_to_mnemonic
from aedge.util.merkle_set import MerkleSet
from aedge.util.prev_transaction_block import get_prev_transaction_block
from aedge.util.path import mkdir
from aedge.util.vdf_prover import get_vdf_info_and_proof
from tests.wallet_tools import WalletTool
from aedge.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_aedge_config(root_path)
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="aedge-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `aedge generate keys`")
    ...
    async def setup_plots(self):
        ...
        try:
            ...
            plot_keys_2 = PlotKeys(self.farmer_pk, None, encode_puzzle_hash(self.pool_ph, "aec"))
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("AEDGE_ROOT", "~/.aedge/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
