...
from aedge.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from aedge.full_node.full_node_api import FullNodeAPI
from aedge.server.start_farmer import service_kwargs_for_farmer
from aedge.server.start_full_node import service_kwargs_for_full_node
from aedge.server.start_harvester import service_kwargs_for_harvester
from aedge.server.start_introducer import service_kwargs_for_introducer
from aedge.server.start_service import Service
from aedge.server.start_timelord import service_kwargs_for_timelord
from aedge.server.start_wallet import service_kwargs_for_wallet
from aedge.simulator.start_simulator import service_kwargs_for_full_node_simulator
from aedge.timelord.timelord_launcher import kill_processes, spawn_process
...
from aedge.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["aec_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "aec")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["aec_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "aec")
