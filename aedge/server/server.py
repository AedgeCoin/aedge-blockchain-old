...
from aedge.protocols.protocol_state_machine import message_requires_reply
from aedge.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from aedge.protocols.shared_protocol import protocol_version
from aedge.server.introducer_peers import IntroducerPeers
from aedge.server.outbound_message import Message, NodeType
from aedge.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from aedge.util.errors import Err, ProtocolError
from aedge.util.ints import uint16
from aedge.util.network import is_localhost, is_in_network
from aedge.util.ssl import verify_ssl_certs_and_keys
...
class AedgeServer:
    def __init__(
        ...
        aedge_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSAedgeConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSAedgeConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.aedge_ca_crt_path, self.aedge_ca_key_path = aedge_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSAedgeConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.aedge_ca_crt_path, self.aedge_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSAedgeConnection] = None
        try:
            connection = WSAedgeConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSAedgeConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.aedge_ca_crt_path, self.aedge_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSAedgeConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSAedgeConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSAedgeConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSAedgeConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSAedgeConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSAedgeConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSAedgeConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSAedgeConnection, trusted_peers: Dict) -> bool:
