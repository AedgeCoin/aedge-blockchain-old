...
from aedge import __version__
from aedge.consensus.coinbase import create_puzzlehash_for_pk
from aedge.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_aedge_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from aedge.util.bech32m import encode_puzzle_hash
from aedge.util.config import (
    create_default_aedge_config,
    initial_config_file,
    load_config,
    save_config,
    unflatten_properties,
)
from aedge.util.ints import uint32
from aedge.util.keychain import Keychain
from aedge.util.path import mkdir
from aedge.util.ssl import (
...
from aedge.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'aedge keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "aec_target_address" not in config["farmer"]
    stop_searching_for_pool = "aec_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("aec_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("aec_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "aec_target_address" not in config["farmer"]:
        print(
            f"Setting the aec destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["aec_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["aec_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['aec_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "aec_target_address" not in config["pool"]:
        print(f"Setting the aec destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["aec_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["aec_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['aec_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the AEC destination addresses, edit the `aec_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    aedge_ca_crt, aedge_ca_key = get_aedge_ca_crt_key()
    aedge_ca_crt_path = ca_dir / "aedge_ca.crt"
    aedge_ca_key_path = ca_dir / "aedge_ca.key"
    write_ssl_cert_and_key(aedge_ca_crt_path, aedge_ca_crt, aedge_ca_key_path, aedge_ca_key)
    ...
    aedge_ca_crt, aedge_ca_key = get_aedge_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, aedge_ca_crt, aedge_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if aedge_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return aedge_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def aedge_version_number() -> Tuple[str, str, str, str]:
...
def aedge_minor_release_number():
    res = int(aedge_version_number()[2])
    print(f"Install release number: {res}")
    return res


def aedge_full_version_str() -> str:
    major, minor, patch, dev = aedge_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def aedge_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
    ...
    """
    if os.environ.get("AEDGE_ROOT", None) is not None:
        print(
            f"warning, your AEDGE_ROOT is set to {os.environ['AEDGE_ROOT']}. "
            f"Please unset the environment variable and run aedge init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Aedge directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if AEDGE_ROOT is set, or if user has run aedge init twice
        # before a new update.
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'aedge keys show --show-mnemonic-seed'")
