...
from aedge.cmds.configure import configure_cmd
from aedge.cmds.farm import farm_cmd
from aedge.cmds.init import init_cmd
from aedge.cmds.keys import keys_cmd
from aedge.cmds.netspace import netspace_cmd
from aedge.cmds.passphrase import passphrase_cmd
from aedge.cmds.plots import plots_cmd
from aedge.cmds.show import show_cmd
from aedge.cmds.start import start_cmd
from aedge.cmds.stop import stop_cmd
from aedge.cmds.wallet import wallet_cmd
from aedge.cmds.plotnft import plotnft_cmd
from aedge.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from aedge.util.keychain import set_keys_root_path, supports_keyring_passphrase
from aedge.util.ssl import check_ssl
...
@click.group(
    help=f"\n  Manage aedge blockchain infrastructure ({__version__})\n",
    epilog="Try 'aedge start node', 'aedge netspace -d 192', or 'aedge show -s'",
...
@cli.command("version", short_help="Show aedge version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs aedge daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by aedge start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from aedge.daemon.server import async_run_daemon
