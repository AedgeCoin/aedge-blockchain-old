...
from aedge.cmds.units import units
...
from aedge.util.misc import format_bytes
from aedge.util.misc import format_minutes
from aedge.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total aedge farmed: {amounts['farmed_amount'] / units['aedge']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['aedge']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['aedge']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'aedge start wallet' and 'aedge wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'aedge wallet show'")
    else:
        print("Note: log into your key using 'aedge wallet show' to see rewards for each key")
