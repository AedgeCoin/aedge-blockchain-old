...
from aedge.daemon.keychain_proxy import (
    KeychainProxy,
...
from aedge.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from aedge.protocols import wallet_protocol
from aedge.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from aedge.util.byte_types import hexstr_to_bytes
from aedge.util.check_fork_next_block import check_fork_next_block
from aedge.util.errors import Err, ValidationError
from aedge.util.ints import uint32, uint128
from aedge.util.keychain import Keychain
from aedge.util.lru_cache import LRUCache
from aedge.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from aedge.util.path import mkdir, path_from_root
from aedge.wallet.block_record import HeaderBlockRecord
from aedge.wallet.derivation_record import DerivationRecord
from aedge.wallet.settings.settings_objects import BackupInitialized
from aedge.wallet.transaction_record import TransactionRecord
from aedge.wallet.util.backup_utils import open_backup_file
from aedge.wallet.util.wallet_types import WalletType
from aedge.wallet.wallet_action import WalletAction
from aedge.wallet.wallet_blockchain import ReceiveBlockResult
from aedge.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'aedge keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSAedgeConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSAedgeConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSAedgeConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSAedgeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSAedgeConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSAedgeConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSAedgeConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
