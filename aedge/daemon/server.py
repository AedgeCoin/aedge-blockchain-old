...
from aedge.cmds.init_funcs import check_keys, aedge_init
from aedge.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from aedge.daemon.keychain_server import KeychainServer, keychain_commands
from aedge.daemon.windows_signal import kill
from aedge.server.server import ssl_context_for_root, ssl_context_for_server
from aedge.ssl.create_ssl import get_mozilla_ca_crt
from aedge.util.aedge_logging import initialize_logging
from aedge.util.config import load_config
from aedge.util.json_util import dict_to_json_str
from aedge.util.keychain import (
    Keychain,
...
from aedge.util.service_groups import validate_service
from aedge.util.setproctitle import setproctitle
from aedge.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Aedge.")
    quit()
...
service_plotter = "aedge plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "aedge": "aedge",
        "aedge_wallet": "start_wallet",
        "aedge_full_node": "start_full_node",
        "aedge_harvester": "start_harvester",
        "aedge_farmer": "start_farmer",
        "aedge_introducer": "start_introducer",
        "aedge_timelord": "start_timelord",
        "aedge_timelord_launcher": "timelord_launcher",
        "aedge_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("aedge daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered AEDGE_ROOT
    os.environ["AEDGE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with AEDGE_ROOT: {os.environ['AEDGE_ROOT']}")
    ...
    if service_command == "aedge_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in aedge_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    aedge_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("aedge_daemon")
